...
// Copyright (c) 2016-2017 The Gallion developers
...
void SendFrame::parsePaymentRequest(QString _request) {
    if(_request.startsWith("gallion://", Qt::CaseInsensitive))
    {
       _request.replace(0, 13, "gallion:");
    }
    if(!_request.startsWith("gallion:", Qt::CaseInsensitive)) {
      QCoreApplication::postEvent(&MainWindow::instance(), new ShowMessageEvent(tr("Payment request should start with gallion:"), QtCriticalMsg));
      return;
    }
 ...
 ConfirmSendDialog dlg(&MainWindow::instance());
    ...
    if (dlg.exec() == QDialog::Accepted) {
      ...
      if (m_ui->donateCheckBox->isChecked()) {
          CryptoNote::WalletLegacyTransfer walletTransfer;
          walletTransfer.address = "gmtcSNyLfTJjEFb3pMDsBagp4XFaaBybiVGyr4Hr23EdQvoUWPVFwhjdjJ7vWnbWYe5jLEGAkVfhLcftvg8YE6r63qqARZ93GK";
