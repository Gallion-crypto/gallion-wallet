...
// Copyright (c) 2016 The Gallion developers
...
const QString BITCOIN_IPC_PREFIX("gallion:");

static QString ipcServerName()
{
    QString name("Gallion");
...
PaymentServer::PaymentServer(QApplication* parent) : QObject(parent), saveURIs(true)
{
    ...
    if (!uriServer->listen(name))
        qDebug() << tr("Cannot start gallion: click-to-pay handler");
    else
...
{
    // clicking on gallion: URLs creates FileOpen events on the Mac:
